...
**Verification (share)**: `gradlew testDebugUnitTest lintDebug assembleDebug` green across all 4 ABIs. On the emulator, tapping "Share log file" opens the system share sheet showing **"Sharing 1 file — zapret-log-2026-08-06.log"**, confirming `query` reports the name correctly and the system treats it as an attachment. `adb shell content read --uri content://dev.zapret.mobile.logs/zapret-log-2026-08-06.log` returns the log contents, confirming `openFile`. Two negative cases confirmed on-device: a percent-encoded `../../shared_prefs/engine_settings.xml` traversal is refused with `Could not resolve`, and an unknown filename with `No such shared log`.

## Post-report update: Russian localisation, language toggle, single round power button (v0.1.8)

Three requested changes to the UI.

**Russian is now the app's own default, independent of the system locale.** `values-ru/strings.xml` translates every user-facing string; `values/strings.xml` stays English as the fallback for every other locale. `app_name` and `language_switch_to` are marked `translatable="false"` -- the first is a product name, the second is a passthrough format carrying the target language's own name.

Per-app language needs `LocaleManager` (API 33) or AppCompat, and this app has neither (minSdk 26, no library dependencies). So `LanguageSettings.wrap(Context)` builds a locale-configured context and every component that reads strings calls it from `attachBaseContext`: all three Activities and `ZapretVpnService` (whose notification channel and actions are user-visible too).

**Language toggle**: a card at the top of Settings with a single button labelled with the language it switches *to*, written in that language -- "English" while Russian is active, "Русский" while English is. Someone who cannot read the current language can still read the way out. Tapping saves the choice and calls `recreate()`.

- **Bug found and fixed during on-device testing**: switching the language in Settings left MainActivity and StrategiesActivity rendering the old language, because both re-render on `onResume` via `setContentView` while string resolution is fixed by the context built in `attachBaseContext`. Both now compare the saved language against the one they were created with and call `recreate()` instead. Caught only by actually switching and walking back to the main screen -- the build was clean and Settings itself looked correct.

**One round power button replaces the separate Start/Stop pair.** `UiKit.powerButton` draws an oval `GradientDrawable` with a soft ring, plus `UiKit.circle` for square layout params so it stays a circle rather than an ellipse. Built on `TextView`, not `Button`: Button's platform background insets fight a fixed square size and visibly clip the oval. The accent colour with "ПУСК"/"START" means stopped, red with "СТОП"/"STOP" means running, and a caption underneath says what a tap will do.

State comes from the new `ZapretVpnService.isRunning()` (`runningInstance` is set once the tunnel is established and cleared as the first step of stopping, so it doubles as live state without binding to the service). `onResume` re-syncs from it, so the control stays honest when the service was started or stopped elsewhere -- a notification action, `onRevoke`, a crash. Tapping start flips the control optimistically, since the tunnel comes up asynchronously.

**Verification**: `gradlew testDebugUnitTest lintDebug assembleDebug` green across all 4 ABIs (lint initially failed on `MissingTranslation` for `app_name`, fixed properly with `translatable="false"` rather than a baseline). Walked through on the emulator with screenshots: fresh install comes up in Russian with the green ПУСК circle; tapping it starts the tunnel (VPN key icon in the status bar) and the circle turns red with "СТОП" and "Нажмите, чтобы выключить защиту"; the Settings toggle switches to English and back, and after the fix the main and strategies screens follow live without restarting the app; the language survives an app restart; the notification channel is created as "VPN Zapret Mobile", confirming the Service picks up the locale too.

## Post-report update: dark theme fixed properly and made the default (v0.1.9)

The earlier dark-theme fix (the theme-aware Spinner adapter) treated one symptom. The cause was structural: `styles.xml` gave every palette the same platform theme, `Theme.Material.Light` with a near-black `textColorPrimary`. The in-app `AppTheme` enum colours what the app draws itself, but everything the *platform* draws -- dialogs, spinner dropdowns, EditText carets and selection handles, and any view whose colour the app doesn't set -- took its colours from that Light theme, so on the dark palette it rendered near-black on near-black.

- Added `AppTheme.Dark` (parent `Theme.Material.NoActionBar`) alongside the existing light `AppTheme`, with the Midnight colours duplicated into `colors.xml` because XML styles cannot read the Java enum. Each `AppTheme` value now names its platform theme via a new `styleResource` field, applied with `UiKit.applyTheme` before `super.onCreate` in all three Activities.
- **A second, separate bug found by looking at the result**: the header banner drew its title in `theme.onAccent`. On Midnight that is `#10151A` -- the correct colour for text on the *bright green accent* (the power button), but the banner is painted with the *gradient*, which on Midnight is nearly black. The app's own title was therefore near-black on near-black. Split the two roles: a new `onGradient` field (light on Midnight, white on the light palettes) now colours the banner title and subtitle, while `onAccent` keeps its original meaning.
- Status and navigation bars are now painted at runtime from the active palette (`UiKit.applyWindowChrome`) instead of being hardcoded to the light surface colour in `styles.xml`, where they left a white status bar above a dark app. Status bar icons stay light in every palette, since all three gradients are dark enough for white.
- **Crash found and fixed during testing**: `applyWindowChrome` was first called before `setContentView` and crashed on launch with an NPE from `getInsetsController()` -- the insets controller belongs to the decor view, which does not exist until there is a content view. Moved after `setContentView` in all three Activities.
- Switching palette now calls `recreate()` rather than rebuilding views, since a platform theme can only be applied before the content view exists -- the same constraint as the language switch. `MainActivity` and `StrategiesActivity` also recreate when they notice a palette change on resume.
