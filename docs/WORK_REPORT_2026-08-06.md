...
**Verification**: `cargo test` 21/21 (unchanged pass count; the affected code path isn't independently unit-tested against a real TLS peer, since that requires a live server -- this is exactly why the live-device auto-test mattered). `gradlew test lintDebug assembleDebug` green across all 4 ABIs. Not yet re-verified against real Discord/YouTube traffic on-device at the time of writing; the fix follows directly from the BoringSSL error signature and TLS 1.3 transcript-hash mechanics, but live confirmation (via the auto-tester or ordinary use) is the actual proof and should be re-run after this release installs.
